\subsection{Brainstorming (21.09.2015)}
	...
	\textit{\textbf{Preview:}} Every member of our team had carefully read the rules of the FTC competition when they were relased. Today we gathered together to discuss all the aspects of this years gameplay and think of how to get on with the most significant features of the game. \newline \newline
	...
	\begin{table}[H]
		...
		\begin{center}
			\begin{tabular}{|p{0.4\linewidth}|p{0.5\linewidth}|p{0.1\linewidth}|}
				...
				\hline
				Moving to the ramp is essential to achieving a high score. & The obot's wheel base should be good at moving on the ramp. & chassis \\
				\hline
				...
				\hline
				Goals for debris have a very small capacity. & It is more preferable to collect cubes to balls. That's why we need mechanism to prevent ball collection. & gripper \\
				\hline
				...
				\hline
				It's quite inconvenient to exchange ramps with your ally during the game. & We will establish who operates where before each game with our allies. Additionally, there should be two autonomus programs for climbing onto both ramps. & strategy \\
				\hline
				The only main difficulty of this year autonomus period is that both robots in alliance have to fulfil the same tasks in the same place. So, there is a high risk of collisions between them. & A number of different programs for autonomus period are needed for easier adjustment to the ally's strategy. & strategy \\
				\hline
	...
	\begin{enumerate*}
		...
		Additionally, the wheel base should be symmetrical with respect to the central axis for stable climbing to the mountain.
		...
		\end{figure}
		% % % %
		\item To score in the high zone goal from 2-nd zone robot should have a mehanism for delivering debris to the distance of 40 cm or more. Shooting debris is an entirely unsuitable approach, because it's impossible to assure enough accuracy for stable scoring, especially with balls. Another way is an elevator. There are three types of lifts which familiar to us: they're crank lift, scissor lift and retractable rails. 
		
		The scissor lift is not suitable for this year competition, because despite it's main advantage - the ability of extracting the longest distances of all - it's the most difficult to develop.
		
		The crank lift allows us to vary the angle of turning of each segment. However, it requires at least one DC motor or a strong servo for every joint.
		
		The retractable rails can only move along one axis. However, they require the least space and can be equipped by one DC motor (as all the motors are connected to the only reel, which winds the cable).
		\begin{figure}[H]
			\begin{minipage}[h]{1\linewidth}
		...
		% % % %
		\item Wheel bases which are good at climbing mountains are usually less manoeuvrable than carriages with omni and mechanium wheels. This way, the less robot will turn, the more effectively it will compete.
